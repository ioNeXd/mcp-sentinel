...
`resources/list`, `resources/read`, `prompts/list` e `prompts/get` com dados
fixos. Não depende de nenhuma biblioteca externa.
...
def main() -> None:
    for raw_line in sys.stdin:
        ...
        try:
            ...
            continue  # ignora lixo no stdin
        if request.get("id") is None:
            continue  # notificacao: nao responde
        try:
            result = handle_request(request)
        except MethodError as exc:  # erro JSON-RPC com código próprio
            response = {
                "jsonrpc": "2.0",
                "id": request.get("id"),
                "error": {"code": exc.code, "message": exc.message},
            }
        except Exception as exc:  # responde erro JSON-RPC em vez de morrer
            response = {
                "jsonrpc": "2.0",
                "id": request.get("id"),
