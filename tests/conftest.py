...
def spawn_fake_server(script: Path, *args: str) -> tuple[int, Any]:  
    ...
    antes de abrir a porta (ex.: argumento inválido): captura o exit code na  
    mensagem de erro para não mascarar a causa num timeout genérico de "não  
    ficou pronto", e tenta nova porta.  
    """  
    import subprocess  
...
def wait_for_port(port: int, process: Any) -> bool:  
    """Aguarda a porta aceitar conexões, abortando se o processo morrer antes."""  
...
class FakeClient(BaseClient):  
    ...
    async def send_request(self, method: str, params: dict[str, Any] | None = None) -> Any:  
        """Responde os métodos MCP conhecidos; ``hang_method`` nunca retorna.  
  
        Quando ``method == hang_method`` a coroutine bloqueia indefinidamente  
        (o timeout é responsabilidade de quem chama — manager/monitor).  
        """  
...
def make_fake_manager(  
    ...
    *,  
    ...
    A fábrica interna de clients (StdioClient) é substituída pela fake via  
    atribuição direta de ``_create_client``.  
    """  
