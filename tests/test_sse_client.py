...
async def test_stream_que_cai_falha_requests_pendentes() -> None:  
    ...
    O ``--delay 2`` mantém o request no ar tempo suficiente para o stream  
    cair primeiro, provando que a queda é detectada e falha a pending na  
    hora — não fica presa até o timeout de 10s.  
    """  
...
async def test_stop_cancela_stream_e_idempotente(sse_backend: str) -> None:  
    """stop() cancela o reader do stream e é idempotente (2ª chamada é no-op)."""  
    client = make_client(sse_backend)  
    await client.start()  
...
async def test_reconexao_recaptura_novo_endpoint(sse_backend: str) -> None:  
    ...
    Nova conexão gera novo session_id no fake, logo novo endpoint anunciado.  
    """  
    client = make_client(sse_backend)  
    await client.start()  
...
async def test_get_do_stream_pede_sse_e_post_pede_json(sse_backend: str) -> None:  
    """GET do stream pede text/event-stream; POST fala application/json.  
...
async def test_post_com_erro_http_falha_imediatamente(sse_backend: str) -> None:  
    """POST respondido 4xx/5xx: erro na hora, sem esperar o timeout.  
  
    O SSE client usa BackendHttpStatusError (subclasse de  
    BackendDisconnectedError) para diferenciar programaticamente 401/404/500  
