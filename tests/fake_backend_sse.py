...
def create_app(  
    *, send_endpoint: bool = True, silent: bool = False, keepalive: float = KEEPALIVE_INTERVAL_SECONDS  
    ...
    Estado por app: ``queues`` mantém uma fila por conexão de stream aberta  
    (limpa quando a conexão cai) e ``sessions`` guarda os session_ids das  
    conexões vivas, usados para validar o POST no modo com endpoint (espelha  
    servidores reais que amarram o POST à sessão).  
    """  
    ...
    async def sse() -> StreamingResponse:  
        """Abre o stream SSE: anuncia o endpoint e drena a fila da conexão.  
  
        No modo ``silent`` o stream fica mudo (exercita o timeout do cliente).  
        Caso contrário, o PRIMEIRO evento é o anúncio ``endpoint`` (spec  
        HTTP+SSE do MCP) com o session_id próprio da conexão; em seguida  
        entrega as respostas da fila e emite keep-alives no timeout.  
        """  
    ...
    async def messages(request: Request) -> JSONResponse:  
        """Recebe o POST JSON-RPC e publica a resposta no stream.  
  
        No modo com endpoint, valida o ``session_id`` da query contra as  
        sessões vivas e responde ``404`` quando ausente/desconhecido — como  
        servidores MCP-SSE reais fazem com a rota errada. Notificações apenas  
        confirmam o recebimento; requests comuns têm a resposta de  
        ``fake_logic.build_response`` enfileirada para todas as conexões.  
        """  
        if send_endpoint:  
