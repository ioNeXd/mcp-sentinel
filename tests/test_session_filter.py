...
async def test_sem_sessao_comportamento_identico_ao_anterior() -> None:
    ...
    Não-regressão: sem filtro a view expõe TUDO e qualquer backend pode ser
    chamado, exatamente como nas fases anteriores.
    """
...
async def test_sessao_sem_filtro_vira_acesso_total() -> None:
    ...
    get_active_backends confirma o estado sem filtro (active_backends None,
    filtered False).
    """
    server, _ = make_server()
    await server.start()
...
async def test_set_active_backends_sem_sessao_retorna_invalid_request() -> None:
    """set_active_backends sem Mcp-Session-Id: -32600 (Invalid Request)."""
    server, _ = make_server()
    await server.start()
    try:
        response = await rpc(
...
async def test_set_active_backends_validacao_de_params() -> None:
    """Params inválidos viram -32602; backend desconhecido falha EXPLÍCITA.

    Nunca há filtro parcial silencioso: um nome inexistente na lista rejeita
    a chamada inteira e o filtro NÃO é aplicado.
    """
    server, _ = make_server()
    await server.start()
    try:
...
async def test_set_retorna_lista_e_get_confirma() -> None:
    """set devolve a lista (ordenada) e get confirma o mesmo filtro."""
    server, _ = make_server()
    await server.start()
    try:
        response = await rpc(
...
async def test_clear_volta_a_ver_tudo() -> None:
    """clear_active_backends remove o filtro: a sessão volta a ver todos."""
    server, _ = make_server()
    await server.start()
    try:
        await rpc(
...
async def test_filtro_aplica_a_tools_resources_e_prompts() -> None:
    """O filtro vale para as três listagens: tools, resources e prompts."""
    server, _ = make_server()
    await server.start()
    try:
        await rpc(
...
async def test_tools_call_fora_do_filtro_erro_idem_inexistente() -> None:
    ...
    Mesmo código e mesma FORMA de mensagem: o filtro não vaza a existência —
    para a sessão, uma tool bloqueada é indistinguível de uma inexistente (só
    o nome da tool difere na mensagem). resources/read e prompts/get idem. A
    tool do backend PERMITIDO continua funcionando.
    """
    server, _ = make_server()
    await server.start()
    try:
...
async def test_filtro_reflete_backend_removido_do_registry() -> None:
    ...
    try:
        ...
        ]  # só a nativa (não pertence a backend); view filtrada fica vazia, sem erro
    finally:
        await server.stop()


def test_sessao_expira_apos_ttl_e_volta_a_ver_tudo() -> None:
    """Expirado o TTL sem atividade, o filtro cai e a sessão volta a ver tudo.

    O acesso dentro do TTL RENOVA o deadline (janela deslizante): só expira
    após um intervalo cheio sem nenhum acesso.
    """
    clock = FakeClock()
...
def test_atividade_renova_o_ttl() -> None:
    """Acessos periódicos (< TTL) renovam o deadline: a sessão nunca expira."""
    clock = FakeClock()
    sessions = SessionFilter(ttl_seconds=60.0, clock=clock)
    sessions.set_active_backends("s", frozenset({"backend-a"}))
...
def test_purge_remove_apenas_expiradas() -> None:
    """purge_expired remove só as sessões vencidas, preservando as ativas."""
    clock = FakeClock()
    sessions = SessionFilter(ttl_seconds=60.0, clock=clock)
...
def test_default_session_ttl_seconds() -> None:
    """O TTL default do GatewayConfig é DEFAULT_SESSION_TTL_SECONDS (3600s)."""
