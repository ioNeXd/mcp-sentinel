...
# ----------------------------------------------------------------------
# Dashboard interativo (Fase 7) — server-rendered no primeiro paint,
# depois assume via JavaScript vanilla (fetch para refresh/ações,
# EventSource para o console de logs ao vivo). Sem build step, sem
# dependência externa de front-end — tudo num único arquivo, como o
# dashboard read-only da Fase 4 que este substitui.
# ----------------------------------------------------------------------
...
def _render_dashboard(
    ...
    *,
    ...
    (só o necessário para o próprio navegador reautenticar suas chamadas —
    nunca logado, nunca em outro lugar do HTML).
    """
    ...
    token_js = json.dumps(token)  # None -> null; string -> "escaped" (JSON é seguro em <script>)
...
def _render_backend_cards(servers: list[dict[str, Any]]) -> str:
    """Cards iniciais (server-rendered) — o mesmo shape que ``renderCard`` no
    JS produz, para o primeiro paint não ficar em branco antes do refresh.
    """
    if not servers:
...
def _validate_new_backend_payload(payload: Any) -> str | None:
    """Valida o corpo de ``POST /api/config/backends``; devolve a mensagem
    de erro (ou ``None`` se válido). Regras espelham o que ``BackendConfig``
    exige: stdio precisa de ``command``; http/sse precisam de ``url``.
    """
...
def _build_backend_entry(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    já validado — mesmo shape das entradas existentes (backends stdio não
    carregam a chave ``type``, igual ao ``backend-a`` do config original).
    """
...
def create_app(
    ...
    *,
    ...
    async def dashboard(request: Request) -> Response:
        ...
        # query) é reembutido no HTML como constante JS — é o mesmo token que
        # o usuário já digitou/colou na URL; nunca um segredo novo exposto.
    ...
    async def add_backend_config(request: Request) -> JSONResponse:
        ...
        name = payload["name"].strip()
        # Mesma env var e default do main.py (DEFAULT_CONFIG_PATH) — assim o
        # painel sempre acha o MESMO arquivo que o Gateway realmente carregou
        # no boot, mesmo rodando com MCP_GATEWAY_CONFIG customizado.
        ...
        # Sobe o backend na hora (pedido do usuário): reaproveita o mesmo
        # caminho de start do boot, via BackendManager.add_backend(). Uma
        # falha aqui não desfaz a gravação em disco nem derruba a request —
        # o backend fica OFFLINE e o HealthMonitor assume o restart, igual a
        # qualquer outro backend que nasce indisponível.
        live_note = ""
        try:
            ...
            # Corrida rara: alguém adicionou o mesmo nome entre a checagem
            # de duplicata acima e aqui. O config.json já foi gravado; só
            # avisa que o start ao vivo não rolou.
    ...
    async def logs_stream(request: Request) -> StreamingResponse:
        ...
        async def event_source() -> Any:
            try:
                # Replay do buffer recente primeiro, pra quem acabou de abrir
                # o dashboard já ver algum contexto em vez de tela vazia.
                for line in replay:
                    yield f"data: {line}\n\n"
                while True:
