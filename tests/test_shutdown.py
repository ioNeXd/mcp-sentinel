...
async def test_main_shutdown_completo_sem_traceback(monkeypatch: pytest.MonkeyPatch) -> None:  
    ...
    Verifica os marcadores de shutdown na saída (interrupted/backend_manager_stopped/  
    complete) e a ausência de traceback e de ``CancelledError`` cru.  
    """  
    ...
    assert "backend_manager_stopped" in output  
...
async def test_graceful_shutdown_propaga_cancelamento_do_chamador(  
    ...
    meio impediria os backends de serem finalizados. O ``session_purger`` não  
    é iniciado de propósito: seu ``stop()`` é um no-op limpo (task is None) e  
    serve só para preencher a etapa intermediária do ``_graceful_shutdown``.  
    """  
...
def test_gateway_subprocess_shutdown_sem_traceback() -> None:  
    ...
    Confirma que não vaza ``Traceback`` nem ``CancelledError`` cru e que os  
    backends são finalizados (``backend_manager_stopped``) antes de sair.  
    """  
