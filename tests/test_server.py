...
# ----------------------------------------------------------------------
# tools/list e tools/call (comportamento da Fase 0, erros da Fase 1)
# ----------------------------------------------------------------------
...
async def test_tools_list_agregado() -> None:
    ...
    # O schema original é preservado (descrição/inputSchema).
...
async def test_tools_call_roteia_para_backend_certo() -> None:
    ...
    # O backend certo recebeu o nome ORIGINAL da tool (sem namespace).
    assert ("tools/call", {"name": "echo", "arguments": {"text": "oi"}}) in client_a.requests
    # No startup o McpServer pede os três tipos de listagem do outro backend.
...
async def test_resources_list_agregado_com_namespace_de_uri() -> None:
    ...
    # A URI original vira backend.<uri>; backend-b (sem resources) não contribui.
...
async def test_resources_read_roteia_uri_original_e_namespaceia_contents() -> None:
    ...
    # O conteúdo devolvido ao cliente traz a uri re-namespaced (round-trip).
