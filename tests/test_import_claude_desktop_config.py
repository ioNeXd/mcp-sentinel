...
# ----------------------------------------------------------------------
# Casos NÃO conversíveis — sinalizados, nunca convertidos errado
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# Sanitização de nomes e colisões
# ----------------------------------------------------------------------
...
def test_nome_com_pontos_e_espacos_e_sanitizado(tmp_path: Path) -> None:
    ...
    # E o nome sanitizado é aceito pela validação do Gateway.
    from gateway.config import GatewayConfig
...
# ----------------------------------------------------------------------
# CLI: política de gravação (--stdout/--output/--force)
# ----------------------------------------------------------------------
...
def test_cli_nunca_sobrescreve_sem_force(
    ...
    exit_code, _, _ = _run_cli(source, capsys, "--force")
    assert exit_code == 0
...
def test_cli_stdout_imprime_json_sem_gravar(
    ...
    assert exit_code == 0
    payload = json.loads(out)  # stdout é JSON puro (avisos vão no stderr)
