...
(extensão `gateway/sessions.py`), controle manual de backends, dashboard e  
...
Há um teto de sessões simultâneas (constante interna `DEFAULT_MAX_SESSIONS` em  
`gateway/sessions.py`, hoje `256` — não é um campo do `config.json`): ao lotar,  
a sessão com deadline mais antigo é evictada. A limpeza de sessões abandonadas  
...
(avisos vão para o stderr, então a saída continua sendo JSON puro). A gravação  
é atômica (arquivo temporário + `os.replace`): uma falha no meio da escrita  
nunca deixa o destino truncado. Se nada for convertido, o comando falha com  
erro em vez de gerar um config vazio.  
...
- **Métricas (Prometheus/OpenTelemetry): adiadas** até o número de backends  
  ...
  manutenção sem uso (princípio do projeto: não introduzir dependência sem  
  necessidade clara). O diagnóstico prático desta fase é o  
  `GET /api/tools/size`.  
- O filtro seletivo é extensão do Gateway (`gateway/sessions.py`) — o método  
