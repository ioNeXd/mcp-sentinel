...
def test_primeiro_paint_e_versao_reduzida_dos_cards() -> None:
    ...
    html_out = _render_dashboard(
        {"status": "ok", "backends": {}, "tools_count": 0, "resources_count": 0, "prompts_count": 0},
        [
            {
                "name": "backend-a",
                "type": "stdio",
...
async def test_auth_ok_com_token_via_query_string() -> None:  
    ...
    try:  
        ...
        payload = resp.json()  
        assert payload.get("error") is None  
        assert {t["name"] for t in payload["result"]["tools"]} == {"backend-a.echo", "backend-b.add", DIAGNOSTIC_TOOL_NAME}  
    finally:  
        await server.stop()  
...
async def test_add_backend_valida_contra_o_schema_antes_de_gravar() -> None:
    ...
    with open(config_path, "w", encoding="utf-8") as f:
        ...
        json.dump(
            {"backends": [{"name": "backend-a", "command": str(FAKE_BACKEND_PATH)}]}, f
        )
...
async def test_import_claude_desktop_valida_o_config_mesclado_antes_de_gravar() -> None:
    ...
    try:
        ...
        async with await make_client(BASE_URL, app) as client:
            ...
            resp = await client.post(
                "/api/import/claude-desktop", json={"path": source_path}
            )
