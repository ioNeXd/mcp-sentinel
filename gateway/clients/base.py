...
class BaseClient(ABC):
    ...
    def _register_pending(self, request_id: int | str) -> asyncio.Future[Any]:
        ...
        if request_id in self._pending:
            raise BackendStateConflictError(f"{self._backend_name()}: id {request_id} já pendente")
    ...
    def _apply_response(self, message: Any) -> bool:
        ...
        if message.get("jsonrpc") != "2.0":
            ...
            self._reject_pending(
                request_id,
                BackendError(f"resposta malformada: jsonrpc '{message.get('jsonrpc')}' != '2.0'"),
            )
            return True
    ...
    def _backend_name(self) -> str:
        """Nome do backend para mensagens de erro e log.

        Usa ``self._config.name`` quando o transporte tem config (todos os
        clients concretos: stdio/http/sse). Clients sem config — dummies de
        teste — caem no nome da classe.
        """
        config = getattr(self, "_config", None)
        return config.name if config is not None else type(self).__name__
    ...
    def _extract_list(result: Any, key: str) -> list[dict[str, Any]]:
        ...
        if key not in result:
            raise BackendListResponseError(f"Resposta de listagem não contém campo '{key}'")
        items = result[key]
        if not isinstance(items, list):
            raise BackendListResponseError(f"Campo '{key}' não é uma lista: {type(items).__name__}")
        return items
