...
def test_estado_de_instancia_nao_e_compartilhado_entre_clients() -> None:  
    """_capabilities/_pending são por instância, não atributo de classe.  
...
async def test_backend_sem_resources_ou_prompts_retorna_lista_vazia() -> None:  
    ...
    Tools continuam funcionando normalmente mesmo quando resources/prompts  
    não são suportados pelo backend.  
    """  
...
async def test_start_falho_no_handshake_nao_deixa_orfaos() -> None:  
    """Handshake que falha encerra processo e tasks (nenhum órfão).  
    ...
    tentativa de start() não deve achar que já está de pé). As asserções  
    conferem que não há processo órfão, que nenhuma task de leitura segue  
    viva e que is_alive() volta False.  
    """  
    client = StdioClient(  
        BackendConfig(  
            name="falho",  
...
async def test_reader_morto_invalida_is_alive() -> None:  
    """Erro inesperado no leitor de stdout: is_alive() vira False.  
  
    O processo segue tecnicamente vivo, mas nenhuma resposta seria processada:  
    o client não pode aparentar saudável para o Health Monitor. Força-se a  
    flag _reader_failed (mesmo estado que _read_stdout setaria) e valida-se a  
    semântica de is_alive() com o processo ainda em execução (cenário zumbi).  
    """  
    client = make_client()  
    await client.start()  
    try:  
        ...
        client._reader_failed = True  # noqa: SLF001  
        assert not client.is_alive()  
...
async def test_reader_task_marcada_como_falha_em_erro_real() -> None:  
    """_read_stdout seta _reader_failed no except (comportamento real).  
  
    Um backend que escreve no stdout e fecha sem quebrar nada não dispara o  
    caminho de erro; para exercitá-lo de verdade, fecha-se o stdout à força  
    (simula falha de transporte no meio da leitura) — o fake continua vivo,  
    só o canal de leitura quebra. Se o readline levantou, _reader_failed=True  
    e is_alive() não pode continuar True.  
    """  
    client = make_client()  
    await client.start()  
    try:  
...
async def test_write_levanta_quando_client_encerrado() -> None:  
    """_write checa _closed proativamente, não apenas no send_request.  
    ...
    fechado. Agora _write levanta cedo: após stop(), _closed=True e stdin  
    fechado, então nenhuma I/O é tentada.  
    """  
    client = make_client()  
    await client.start()  
    await client.stop()  
...
async def test_send_notification_levanta_quando_client_encerrado() -> None:  
    """_send_notification delega a _write, que checa _closed.  
